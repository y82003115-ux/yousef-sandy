(function () {
  ...
  function launchSnowstorm(stage){
    const canvas=document.createElement('canvas'),fog=document.createElement('div'),flare=document.createElement('div'),ground=document.createElement('div');canvas.className='cinema-snow-canvas';fog.className='cinema-snow-fog';flare.className='cinema-ice-flare';ground.className='cinema-snow-ground';stage.append(canvas,fog,flare,ground);
    ...
    const flakes=Array.from({length:215},()=>{const z=.15+Math.random()*.95;return{x:Math.random()*innerWidth,y:-Math.random()*innerHeight,r:.55+z*z*8.5,z,a:.2+z*.78,v:.7+z*5.8,spin:Math.random()*6.28,tw:(Math.random()-.5)*.11,seed:Math.random()*8}});
    ...
    const frame=now=>{const t=(now-start)/1000,fadeIn=Math.min(1,t/.9),fadeOut=Math.min(1,(9.2-t)/1.3),life=Math.max(0,fadeIn*fadeOut);if(t>9.3){cancelAnimationFrame(raf);stage.className='cinema-fx-stage';stage.innerHTML='';return}ctx.clearRect(0,0,w,h);const impact=Math.exp(-Math.pow((t-3.8)/1.45,2)),gust=(20+impact*72+Math.sin(t*2.1)*13)*life;flakes.forEach(f=>{f.x+=(gust*f.z+Math.sin(t*2.2+f.seed)*2.2)*(.45+f.z);f.y+=f.v*(.7+life+impact*1.9);f.spin+=f.tw*(1+impact*2);if(f.y>h+25||f.x>w+70){f.y=-30-Math.random()*h*.32;f.x=-80+Math.random()*(w*.72)}if(f.x<-80)f.x=w+30;f.a=(.16+f.z*.78)*life;drawCrystal(f,gust)});raf=requestAnimationFrame(frame)};
