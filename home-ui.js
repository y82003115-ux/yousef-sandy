(function () {
  ...
  function launchSnowstorm(stage){
    const canvas=document.createElement('canvas'),fog=document.createElement('div'),flare=document.createElement('div'),ground=document.createElement('div'),ice=document.createElement('div'),room=document.querySelector('#cinema-room .cinema-room');canvas.className='cinema-snow-canvas';fog.className='cinema-snow-fog';flare.className='cinema-ice-flare';ground.className='cinema-snow-ground';ice.className='cinema-ice-frame';ice.innerHTML='<i class="ice-top"></i><i class="ice-right"></i><i class="ice-bottom"></i><i class="ice-left"></i>';room?.classList.add('snow-frozen');stage.append(canvas,fog,flare,ground,ice);
    ...
    const drawCrystal=(f,gust)=>{ctx.save();ctx.translate(f.x,f.y);ctx.rotate(f.spin);ctx.globalAlpha=f.a;ctx.strokeStyle=f.z>.72?'#ffffff':'#d9f5ff';ctx.fillStyle='#fff';ctx.lineCap='round';ctx.lineWidth=Math.max(.45,f.r*.12);ctx.shadowColor='#7dd8ff';ctx.shadowBlur=f.r*(f.z>.72?2.7:1.2);if(f.r<2.2||Math.abs(gust)>32){ctx.beginPath();ctx.moveTo(-gust*f.z*.34,-f.v*f.z);ctx.lineTo(f.r*.65,f.r);ctx.stroke()}else{for(let k=0;k<6;k++){ctx.rotate(Math.PI/3);ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(f.r,0);if(f.r>4){ctx.moveTo(f.r*.55,0);ctx.lineTo(f.r*.78,-f.r*.22);ctx.moveTo(f.r*.55,0);ctx.lineTo(f.r*.78,f.r*.22)}ctx.stroke()}ctx.beginPath();ctx.arc(0,0,Math.max(.5,f.r*.1),0,7);ctx.fill()}ctx.restore()};
    const frame=now=>{const t=(now-start)/1000,fadeIn=Math.min(1,t/.65),fadeOut=Math.min(1,(10.2-t)/1.45),life=Math.max(0,fadeIn*fadeOut);if(t>10.3){cancelAnimationFrame(raf);room?.classList.remove('snow-frozen');stage.className='cinema-fx-stage';stage.innerHTML='';return}ctx.clearRect(0,0,w,h);const impact=Math.exp(-Math.pow((t-4.1)/1.65,2)),gust=(7+impact*31+Math.sin(t*2.1)*7)*life;flakes.forEach(f=>{f.x+=(gust*f.z+Math.sin(t*2.2+f.seed)*1.8)*(.38+f.z);f.y+=f.v*(.85+life*.95+impact*1.15);f.spin+=f.tw*(1+impact*2);if(f.y>h+30){f.y=-20-Math.random()*90;f.x=Math.random()*w}if(f.x>w+55)f.x=-25;if(f.x<-55)f.x=w+25;f.a=(.2+f.z*.76)*life;drawCrystal(f,gust)});raf=requestAnimationFrame(frame)};
